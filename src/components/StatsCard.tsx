...
const iconMap = { received: ArrowDownLeft, sent: ArrowUpRight, total: DollarSign, balance: TrendingUp };
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-5 animate-fade-in">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className={`rounded-lg p-2 ${isReceived ? "bg-pix-received-bg" : icon === "sent" ? "bg-pix-sent-bg" : "bg-secondary"}`}>
          <Icon size={18} className={isReceived ? "text-pix-received" : icon === "sent" ? "text-pix-sent" : "text-muted-foreground"} />
        </div>
      </div>
